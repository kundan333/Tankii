...
public class Terrain {
	...
	//	System.out.println("this.x : "+this.x+" this.z : "+this.z);
		...
		//total grid -- heights.length -- heightmap is 256*256
	...
	private RawModel generateTerrain(Loader loader){
		...
		HeightsGenerator heightGenerator = new HeightsGenerator();
	...
	private float generateHeight(int x,int z,HeightsGenerator heightGenerator) {
		
		return heightGenerator.generateHeight(x, z);
	}
	
	private Vector3f calculateNormal(int x,int z , HeightsGenerator heightGenerator) {
		
		float heightL = generateHeight(x-1 , z ,heightGenerator);
		float heightR = generateHeight(x+1,z,heightGenerator);
		float heightD = generateHeight(x,z-1,heightGenerator);
		float heightU = generateHeight(x,z+1,heightGenerator);
		Vector3f normal = new Vector3f(heightL - heightR,2f,heightD -heightU);
		normal.normalise();
		return normal;
